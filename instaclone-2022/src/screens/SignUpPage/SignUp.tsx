...
function SignUp() {
  ...
  const onSubmit: SubmitHandler<IForm> = async (data) => {
    ...
    try {
      ...
      const fullName = name.concat(userName) //성+이름
      console.log(fullName)
      await createdUser.user.updateProfile({ //회원가입된 유저 정보추가하기(성,이름,프로필사진)
        displayName: fullName,
        photoURL: `https://picsum.photos/600/600/?random`,
      });
    ...
    <AuthLayout>
       <TopBox>
         ...
         <form onSubmit={handleSubmit(onSubmit)}>
           ...
           <Input {...register("name", { required: "이름을 입력해 주세요" })} hasError={Boolean(errors?.name?.message)} placeholder="성명" type="text"  onChange={(e) => setName(e.target.value)}/>
           <FormError message={errors?.username?.message} />
           <Input {...register("username", { required: "사용자 이름을 입력해 주세요" })} hasError={Boolean(errors?.username?.message)} placeholder="사용자 이름" type="text" onChange={(e) => setUserName(e.target.value)} />
